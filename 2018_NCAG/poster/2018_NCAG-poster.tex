...
\begin{column}{\onecolwid} % The third column
  ...
  \begin{description}
    \item[Bridging] Automatic reformulation of a constraint into an equivalent
      ...
      cone constraint. In particular, reformulates SOS constraints
      into PSD constraints. An interior-point solver that natively supports
      SOS and WSOS (SOS with \texttt{domain}) without reformulation to SDP using
      the approach of \cite{papp2017sum} is under development.
    \item[Caching] Cache of the problem data in case the solver do not support
      a modification (can be disabled). For instance, Mosek provides many
      modification capabilities in the API but CSDPs only support to
