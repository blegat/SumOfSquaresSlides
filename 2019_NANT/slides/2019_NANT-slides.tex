...
\title{L'extension de JuMP à la programmation Sum-of-Squares}
%\date{}
\author{Beno\^it Legat (UCL)\\\emph{Co-auteurs:}\\Chris Coey, Robin Deits, Joey Huchette, Amelia Perry and Tillmann Weisser}
\institute{UCLouvain\\
...
\begin{document}
  \maketitle
  \begin{frame}{Formes positives et sommes de carrés}
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
  \end{frame}
  \begin{frame}{Polynômes positifs et sommes de carrés}
    \begin{tikzpicture}
      \draw[->, bend left=30] (-1, 1.6) node[left] {$(x_1, x_2, x_3)$} to (-.1, 1.3);
      \draw[->, bend left=20] (.6, 1.6) node[above] {$(x_1, x_1x_2, x_2)$} to (.9, 1.35);
      \draw[->, bend right=30] (2.1, 2) node[right] {\alert{\emph{pas} unique}} to (1.55, 1.35);
      \node at (-.1, 1.2) {$p(x)$};
      \node at (.5, 1.2) {$=$};
      \node at (1.3, 1.2) {$X^\Tr Q X$};
...
%      \[ \langle x, A^* y \rangle_2 = \langle A x, y \rangle_1 \]
    ...
    \begin{block}{Transformation de contrainte $f$-in-$S_1$ en contrainte $Af$-in-$S_2$.}
      %Transforme contrainte $f$-in-$S_1$ en $Af$-in-$S_2$.
